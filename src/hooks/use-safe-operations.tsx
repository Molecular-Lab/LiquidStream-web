...
export const useSafeStreamOperations = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            // Get transaction hash for tracking (no signing needed for transaction data)
            const txHash = await txBuilder.getTransactionHash(safeTransaction)
            const transactionData = await txBuilder.getTransactionData(safeTransaction)
...
export const useBatchSafeOperations = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            // For simplicity, use the first transaction's hash
            const txHash = await txBuilder.getTransactionHash(safeTransactions[0])
            const transactionData = await txBuilder.getTransactionData(safeTransactions[0])
